...
def print_operation_table(operation: Callable ,num_rows = 6, num_columns = 6):
    horisont = [i+1 for i in range(num_columns)]
    vertical = [i+1 for i in range(num_rows)]
    for i in range(len(horisont)):
        stroka=[]
        for g in range(len(vertical)):
            stroka.append(operation(horisont[i], vertical[g]))
        print(stroka)
